...
clean_evidence
run_hook "$(bash_input_str 'tmux-companion.sh --review-complete /tmp/f.toon' "$COMBINED_STDOUT")"
assert "String APPROVED → codex evidence created" 'has_evidence "codex"'

echo "=== review-complete: APPROVED via full path ==="
clean_evidence
run_hook "$(bash_input_obj '~/.claude/skills/agent-transport/scripts/tmux-companion.sh --review-complete /tmp/f.toon' "$COMBINED_STDOUT")"
...
clean_evidence
echo "$(bash_input_obj 'tmux-companion.sh --review-complete /tmp/f.toon' "$COMBINED_STDOUT")" \
...
clean_evidence
run_hook "$(bash_input_obj 'tmux-companion.sh --review-complete /tmp/f.toon' 'Error' 1)" || true
...
run_hook "$(bash_input_obj 'tmux-companion.sh --triage-override code-critic "Out-of-scope: rebased auth files"' "$OVERRIDE_STDOUT")"
...
run_hook "$(bash_input_obj 'tmux-companion.sh --triage-override code-critic "No critic ran"' "$OVERRIDE_STDOUT2")"
