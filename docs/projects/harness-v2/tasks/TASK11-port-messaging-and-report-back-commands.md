...
- [x] `party-cli` owns relay, broadcast, read, report, and worker enumeration
- [x] Delivery confirmation semantics match `tmux-codex.sh` contract: send returns success only after target pane receives content (not just after `tmux send-keys`)
- [x] Report-back (`--report`) remains compatible with current worker workflows documented in `claude/skills/party-dispatch/SKILL.md` and `claude/CLAUDE.md`
- [x] Large or multi-line messages route through temp-file indirection when payload exceeds tmux send-keys limits
- [x] Worker enumeration output matches current `party-relay.sh --list` format for backward compatibility during coexistence
- [x] Shell wrapper `party-relay.sh` delegates to `party-cli` without behavior drift
